...
contract RebaseToken is ERC20, Ownable, AccessControl {
    ...
    function setInterestRate(uint256 _interestRate) external onlyOwner {
        if (_interestRate >= s_interestRate) {
            revert RebaseToken__InterestRateCanOnlyDecrease(_interestRate, s_interestRate);
        }
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return (super.balanceOf(account) * _calculateUserAccumulatedInterestSinceLastUpdate(account)) / PRECISION_FACTOR;
    }
